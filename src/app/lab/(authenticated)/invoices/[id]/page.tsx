...
import { deleteInvoice, updateInvoiceStatus } from '@/lib/actions/invoice-actions'
            ...
            <tbody className="divide-y divide-white/10">
              {(invoice.items && invoice.items.length > 0) ? invoice.items.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="py-3 text-right text-white/70 font-sans">${item.rate.toFixed(2)}</td>
                  <td className="py-3 text-right text-white font-medium font-sans">${item.amount.toFixed(2)}</td>
                </tr>
              )) : (
                <tr>
