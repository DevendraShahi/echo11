...
import { LabCard as Card } from '@/components/ui/LabCard'
import { LabButton } from '@/components/ui/LabButton'
import { PageHeader } from '@/components/ui/PageHeader'
import { SearchInput } from '@/components/ui/SearchInput'
import { FilterTabs, FilterTab } from '@/components/ui/FilterTabs'
import { EmptyState } from '@/components/ui/EmptyState'
import { Plus, FileText, Download, Send, DollarSign, AlertCircle, Receipt } from 'lucide-react'
...
export default function InvoicesPageClient({ initialInvoices, initialProjects, forceOpenModal, canEdit = false }: InvoicesPageProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterTabs tabs={statusTabs} activeTab={view} onChange={(id) => setView(id as typeof view)} />
      ...
      {filteredInvoices.length === 0 ? (
        <EmptyState
          ...
          action={!searchQuery && view === 'all' && canEdit ? {
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-white/10">
              {filteredInvoices.map((invoice) => {
                ...
                return (
                <tr key={invoice.id} className={cn("transition-colors", isOverdue ? "bg-rose-500/5 hover:bg-rose-500/10" : "hover:bg-white/5")}>
                  ...
                  <td className={cn("px-6 py-4 text-sm font-sans", isOverdue ? "text-rose-400" : "text-white/70")}>
