  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-white/50 mt-1 font-mono text-sm">Communicate with the echo11 team</p>
      </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-white/30">
                  ...
                  <p className="text-center font-mono text-sm">No messages yet.<br/>Start a conversation with the echo11 team!</p>
                </div>
              ) : (
            ...
            <form onSubmit={handleSend} className="p-4 border-t border-white/10">
              <div className="flex gap-3">
                <input
                  ...
                  disabled={sending}
                  className="flex-1 px-4 py-3 bg-white/5 border border-white/10 text-white placeholder:text-white/30 focus:border-accent focus:outline-none font-mono text-sm transition-all"
                />
                <button
                  type="submit"
                  disabled={sending || !newMessage.trim()}
                  className="px-4 py-3 bg-accent hover:bg-accent/90 text-black font-mono uppercase tracking-wider text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {sending ? (
          ...
          <div className="p-5 bg-[#0a0a0a] border border-white/10">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-white/50 text-sm font-mono">Team Messages</span>
                <span className="text-white font-mono">{teamMessages.length}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-white/50 text-sm font-mono">Your Messages</span>
                <span className="text-white font-mono">{clientMessages.length}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-white/50 text-sm font-mono">Unread</span>
                <span className={`font-mono ${unreadCount > 0 ? 'text-accent' : 'text-white/50'}`}>
                  {unreadCount}
                </span>
              </div>
            </div>
          ...
          <div className="p-5 bg-[#0a0a0a] border border-white/10">
            ...
            <ul className="space-y-2 text-sm text-white/40">
              <li className="flex items-start gap-2">
                ...
                <span className="font-mono">Ask about project progress</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-accent">•</span>
                <span className="font-mono">Request deliverables</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-accent">•</span>
                <span className="font-mono">Share feedback</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-accent">•</span>
                <span className="font-mono">Get clarifications</span>
              </li>
            </ul>
          </div>
        </div>
